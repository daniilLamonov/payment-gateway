...
# ============================================
# Dynamic Payment URL CRUD
# ============================================

def get_active_dynamic_url(db: Session) -> Optional[models.DynamicPaymentURL]:
    """Получить активную динамическую ссылку"""
    now = datetime.utcnow()
    return db.query(models.DynamicPaymentURL).filter(
        and_(
...
def get_all_dynamic_urls(db: Session, limit: int = 100) -> List[models.DynamicPaymentURL]:
    """Получить все динамические ссылки"""
    return db.query(models.DynamicPaymentURL) \
...
def create_dynamic_url(
    ...
    """Создать новую динамическую ссылку и деактивировать старые"""

    # Деактивируем все старые ссылки
...
def get_working_hours_by_day(db: Session, day_of_week: int) -> Optional[models.WorkingHours]:
    """Получить рабочее время для конкретного дня"""
    return db.query(models.WorkingHours).filter(
...
def update_or_create_working_hours(
    ...
    """Обновить или создать рабочее время"""
